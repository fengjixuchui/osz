...
namespace :osz do
  ...
  # normal apps
  APP_DEFAULTS = %w( hello chars echo2 bf pipo mon cpuid edit ).collect {|t| make_app(t) }
  ...
  make_kernel( PATH_OS_SYS, make_mod('osz2bt'),
    ...
    [
      make_mod('init'),
      %w( zcom mon hello chars echo2 cpuid ).sort.collect {|t| make_app(t) },
      PATH_TFDISK_BIN
    ].flatten
  )
